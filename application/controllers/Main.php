...
class Main extends CI_Controller {
	...
	function login_validation() {
		...
		if ($this->form_validation->run()) {
			...
			$Url = getenv('SERVICE_HOST').'/api/users/verify-user?UserName='.$UserName.'&Password='.$Password;
			$Header = array('Authorization: Bearer ', 'Content-Type: application/json');
			$ServiceResponse = $this->curlrequest->ServiceRequest($Url, 'GET', $Header, []);
			...
			if($ServiceResponse->Status) {
				...
				if(!$ServiceResponse->Data->Data->UserID) {
					...
					if($getCacheData->Status) {
						...
						set_cookie(getenv('JWT_COOKIE_NAME'), $getCacheData->Data->JwtToken, 3600);
